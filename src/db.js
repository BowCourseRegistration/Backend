...
export const AddCourse = async function (course) {
  const query = `
    INSERT INTO dbo.Courses (courseCode, courseName, startingDate, endingDate, fees, description, termID)
...
export const SearchCourse = async function (keyword) {
    ...
    try {
      return await executeQuery(query, [{name: 'keyword', type: sql.NVarChar, value: keyword}]);
    } catch (err) {
      throw err;
    }
};
...
export const DeleteCourse = async function (courseCode) {
  ...
  try {
    await executeQuery(query, [{ name: 'courseCode', type: sql.Int, value: courseCode }]);
...
export const SearchStudentsByProgram = async function (program) {
  ...
  try {
    return await executeQuery(query, [{ name: 'program', type: sql.NVarChar, value: program }]);
    } catch (err) {
    throw err;
    }
};
...
export const UpdateCourseBycourseCode = async function (courseCode, updateData) {
  const query = `
    UPDATE Course
...
export const StudentLogin = async function (student) {
  ...
  const match = await bcrypt.compare(student.password, result.recordset[0].password);
    if (match) {
        return 'Login success';
    } else {
        throw new Error('Invalid credentials');
